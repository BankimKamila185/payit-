...
const QrScanner = ({ onClose, onScanSuccess, onEnterVpa }) => {
  const [vpa, setVpa] = useState('');
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState('');

  const proceed = async () => {
    if (!onEnterVpa) return;
    setBusy(true); setErr('');
    const res = await onEnterVpa(vpa);
    setBusy(false);
    if (res && !res.ok) setErr(res.error || 'UPI ID nahi mila');
  };
  ...
  return (
    <div style={styles.container} className="animate-slide-up">
      ...
      <div style={styles.header}>
        ...
        <span style={styles.headerTitle}>Pay to UPI ID</span>
        <button style={styles.helpBtn}>
          <HelpCircle size={18} color="#ffffff" />
        </button>
      </div>

      {/* PRIMARY: Pay to any UPI ID (real flow — reach anyone, incl. non-contacts) */}
      <div style={styles.payToCard}>
        <div style={styles.payToLabel}>
          <User size={14} color="var(--accent-neon)" style={{ marginRight: 6 }} />
          Enter any UPI ID to pay
        </div>
        <div style={styles.payToRow}>
          <input
            style={styles.payToInput}
          ...
          />
          <button style={{ ...styles.payToBtn, opacity: busy ? 0.6 : 1 }}
            disabled={busy} onClick={proceed} aria-label="Proceed">
            <ArrowRight size={18} color="#000" />
          </button>
        </div>
        {err && <span style={styles.payToErr}>{err}</span>}
        <span style={styles.payToHint}>Verify hone ke baad fraud engine score karega</span>
      </div>
